...
The sign-in and consent screens use the compact, server-rendered SubKit-style card UI. In local/API-key mode, an existing scoped ContentKit key proves the operator identity; derived OAuth codes and tokens stop working as soon as that source key is revoked, expires or rotates. OAuth access tokens are never accepted as operator keys. In production OIDC mode, administrators first pre-provision the exact provider, issuer and immutable `sub`; email alone never grants access. The initial `WWW-Authenticate` challenge advertises every enabled MCP tier so general-purpose clients request the complete tool surface. Requested scopes are pre-selected, while the operator may reduce the grant within the identity's live role, product-scope and site ceiling; consent never grants a scope the client did not request.
...
CONTENTKIT_OAUTH_LOGIN_PROVIDER=oidc
CONTENTKIT_OAUTH_ALLOWED_SCOPES=mcp:read,mcp:authoring,mcp:admin
CONTENTKIT_OAUTH_DCR_ENABLED=true
CONTENTKIT_OAUTH_OIDC_PROVIDERS=[{"id":"company","label":"Company SSO","issuer":"https://id.example.com","client_id":"...","client_secret":"...","scopes":"openid email profile"}]
```

`CONTENTKIT_OAUTH_SECRET` is mandatory in production and must be independent from the API-key pepper, preview/session secrets and usage HMAC. OIDC issuer URLs must use HTTPS. Production grants are managed with `/v1/identity-grants` or the corresponding MCP admin tool.
