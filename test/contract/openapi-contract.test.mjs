...
test('preview contract separates reusable invitation access from the memorable URL', () => {
  ...
  assert.equal(response.properties.review_targets.items.properties.revision_id.format, 'uuid')
  assert.equal(response.properties.review_targets.items.properties.preview_url.format, 'uri')
  assert.match(response.properties.invitation_url.description, /first changed document/i)
