...
The sign-in and consent screens use the compact, server-rendered `mcp-auth-v2` card, branded `ck`. One API-key plus multiple named direct OIDC adapters may be enabled concurrently. ContentKit configures and operates every client, callback, secret, session and grant itself. The chooser always renders **Continue with SSO** first and **Continue with API key** second; configured adapter labels do not become user-facing actions. In API-key mode, an existing scoped ContentKit key proves the operator identity; derived OAuth codes and tokens stop working as soon as that source key is revoked, expires or rotates. OAuth access tokens are never accepted as operator keys. OIDC identities require `sub` and an exact pre-provisioned `provider_id + issuer + sub` identity grant; email is optional, and only a provider-verified address refreshes the grant. Email alone never grants access. Operator sessions have an eight-hour idle and 24-hour absolute limit, are revalidated, can be explicitly logged out, and the consent page can switch accounts. Requested scopes are pre-selected, while the operator may reduce the grant within the identity's live product-scope and site ceiling (the stored truth of a grant; the named role is only a denormalized display value); consent never displays or grants a scope the client did not request.
...
generic login start, GET callback and POST logout. Authorize redirects to
...
`email` is nullable and never contains an unverified provider claim.
...
CONTENTKIT_OAUTH_PROVIDERS=[{"protocol":"api_key","id":"api-key","label":"ContentKit API key"},{"protocol":"oidc","id":"workforce-oidc","label":"Workforce OIDC","issuer_url":"https://identity.example.com","client_id":"...","client_secret":"...","scopes":"openid email profile"}]
```

`CONTENTKIT_OAUTH_SECRET` is mandatory in production and must be independent from the API-key pepper, preview/session secrets and usage HMAC. `CONTENTKIT_OAUTH_PROVIDERS` is ContentKit's own browser-provider configuration: `api_key` may occur once, named `oidc` records may occur several times, and every id is unique. Issuers must use HTTPS and each deployment registers its own callback. Identity grants are managed with `/v1/identity-grants` or the corresponding MCP admin tool. Product storage credentials remain unrelated to identity. All adapters use only `/v1/identity/login/start`, the GET `/v1/identity/login/callback`, and `/v1/identity/logout`.

Discovery exposes only `protocol`, opaque `id`, canonical `label` (`SSO` or
`API key`) and necessary protocol metadata (`issuer` for OIDC). The
