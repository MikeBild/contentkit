 ...
 * "1–25 of 106" counted client-side over a page the server truncated is a number
 * nobody said. So order and size are modelled here as capabilities that come
 * from the endpoint, not from the component:
 *
 *   `Paging` says whether one response holds every row. `GET /v1/sites/{site}/content`
 *   answers a bare array with no `limit`, no `cursor` and no `order` parameter —
 *   that is `'whole'`. `GET /v1/sites/{site}/published` answers
 *   `{ items, next_cursor }` — that is `'cursor'`.
 *
 ...
 *   only be reordering the window it happens to have, `'none'` when the column
 *   has no order at all. Only the first two may be offered; `'page'` is the lie
 *   and the UI must not render a control for it.
 *
 ...
 * `./cursor` and the same `Pagination` control. A client-minted cursor is still a
 * real cursor: it addresses a position in a result the client fully holds.
 *
...
// as by the bundler, and Node's resolver does not guess one. `tsconfig.json`
...
/**
 * A column, reduced to the four facts that decide what may be offered for it.
 * The rendering half (label, cell, classes) lives with the component; this shape
 * is deliberately data so it can be reasoned about without a DOM.
 */
 ...
 * a parameter with nothing recognisable in it falls back to the default view —
 * a hand-edited or stale link must never open an empty table.
 */
 ...
 * is empty, loading or broken, which is the one distinction the console insists on.
 */
 ...
 *
 * Three states, not two: ascending, descending, and back to the endpoint's own
 * order. The third is a real state and it is not otherwise reachable — the
 * authoring list arrives ordered by `created_at` descending, which is not any
 * column the operator can name, so "undo my sort" has to be a move.
 *
 ...
 *
 * Hiding the sorted column is the case that matters: the rows stay in that
 * order, the arrow that explained it is gone, and the list now looks like the
 * default order while being something else entirely.
 */
...
export function compareTime(left: string | null | undefined, right: string | null | undefined): number {
  ...
  // make the sort comparator return NaN and the order implementation-defined.
 ...
 * disagree: "Page 3" over rows 1–25. Returning the state unchanged by identity
 * in the common case keeps this callable on every render.
 */
