...
async function ensureMigrationsTable(client) {
  ...
  await client.query('CREATE UNIQUE INDEX IF NOT EXISTS contentkit_migrations_tag_idx ON "contentkit"."__contentkit_migrations" (tag) WHERE tag IS NOT NULL')
}
 ...
 * journal without re-running SQL, matching Subkit's production runner.
 */
  ...
  for (const migration of migrations) {
    if (applied.tags.has(migration.tag)) {
      if (applied.hashes.has(migration.hash)) {
        ...
        await client.query(
          'UPDATE "contentkit"."__contentkit_migrations" SET hash = $1 WHERE tag = $2',
        ...
        logger.warn?.('migration hash drift backfilled', {
          tag: migration.tag, hash: migration.hash,
        })
      }
      continue
    }
    ...
    if (applied.hashes.has(migration.hash)) {
      await client.query(
        'UPDATE "contentkit"."__contentkit_migrations" SET tag = $1 WHERE hash = $2 AND tag IS NULL',
...
export async function detectMigrationDrift(client, migrations = EMBEDDED_MIGRATIONS) {
  await ensureMigrationsTable(client)
  const result = await client.query('SELECT id, hash, tag, created_at FROM "contentkit"."__contentkit_migrations" ORDER BY id')
  ...
  return {
    ...
    missing_in_db: migrations.filter((migration) => !dbHashes.has(migration.hash)).map(({ idx, tag, hash }) => ({ idx, tag, hash })),
  }
}
