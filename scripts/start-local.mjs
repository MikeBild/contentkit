...
function ensureLocalPostgres() {
  ...
  if (inspect.status !== 0) {
    const created = docker(
      ...
      container,
      '-e',
      'POSTGRES_PASSWORD=contentkit-local',
      '-e',
      'POSTGRES_DB=contentkit',
      '-p',
      '127.0.0.1:55432:5432',
      '-v',
      'contentkit-local-postgres:/var/lib/postgresql/data',
      'postgres:16-alpine',
    )
...
try {
  ...
  if (config.storageUrl === localBoundaryUrl || config.webhookUrl.startsWith(localBoundaryUrl)) {
