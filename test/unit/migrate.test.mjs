...
class FakeClient {
  ...
  async query(sql, params = []) {
    ...
    if (text === 'BEGIN') { this.transaction = []; return { rows: [] } }
    if (text === 'COMMIT') {
      for (const operation of this.transaction || []) operation()
      this.transaction = null
      return { rows: [] }
    }
    if (text === 'ROLLBACK') { this.transaction = null; return { rows: [] } }
    ...
    if (text.includes('__contentkit_migrations') || text.startsWith('CREATE EXTENSION') || text.startsWith('CREATE SCHEMA')) {
      return { rows: [] }
    }
