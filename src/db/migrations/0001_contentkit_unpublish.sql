-- Adds a subtractive path to releases: ck_activate_release learns
-- p_retire_item_ids, which archives the item's published revision and clears
-- published_revision_id so the item drops out of every future snapshot.
-- The old 2-arg function must be dropped first: adding a defaulted parameter
-- would otherwise create an overload and make 2-arg calls ambiguous.
drop function if exists public.ck_activate_release(uuid, uuid[]);

create or replace function public.ck_activate_release(p_release_id uuid, p_revision_ids uuid[] default '{}', p_retire_item_ids uuid[] default '{}')
returns void
language plpgsql
