...
export async function startLocalBoundary({ host = '127.0.0.1', port = 55433, dataDir, logger = console } = {}) {
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (['GET', 'HEAD'].includes(req.method) && url.pathname.startsWith(downloadPrefix)) {
        ...
        let payload
        try {
          payload = await readFile(safePath(objectsDir, objectName))
        } catch {
      ...
      if (req.method === 'POST' && url.pathname === '/hooks/contentkit-notifications') {
        ...
        logger.info?.(`local webhook: ${req.headers['webhook-type']} (${req.headers['webhook-id']})`)
