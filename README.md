...
It is deliberately not a WYSIWYG CMS: there is no visual page builder and no
plugin system. Content lives as portable Markdown, and the API remains the
contract ([OpenAPI 3.1](docs/openapi.json)). The same self-contained binary also
serves the first-party operator Cockpit at `/cockpit/`; it manages ContentKit's
publishing domain through that API instead of introducing a second content
model or template editor. Contentkit powers personal sites in production.
