...
const TABLES = new Set([
  'ck_sites', 'ck_site_domains', 'ck_site_locales', 'ck_api_keys',
  'ck_content_items', 'ck_content_revisions', 'ck_assets', 'ck_releases',
  'ck_release_entries', 'ck_preview_tokens', 'ck_comments',
  'ck_contact_submissions', 'ck_outbox_events',
])
...
export function createPostgres(config, options = {}) {
  ...
  const db = {
    ...
    async insert(table, body, { returning = true, upsert = false, onConflict } = {}) {
      ...
      let conflict = ''
      if (upsert) {
        const targets = String(onConflict || '').split(',').filter(Boolean)
        if (!targets.length) throw new Error('upsert requires onConflict')
        const targetSql = targets.map(identifier).join(', ')
        const updates = columns.filter((column) => !targets.includes(column))
      ...
      return returning ? (await pool.query(sql, values)).rows : (await pool.query(sql, values), null)
    },
    ...
    async remove(table, filters) {
      ...
      await pool.query(`DELETE FROM ${tableName(table)}${where}`, values)
    },

    async rpc(name, body = {}) {
      if (name !== 'ck_activate_release') throw new Error(`unknown Contentkit function: ${name}`)
      await pool.query('SELECT public.ck_activate_release($1, $2)', [
        body.p_release_id,
        body.p_revision_ids || [],
      ])
      return null
