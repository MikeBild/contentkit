...
test('single binary completes the local CMS lifecycle end to end', {
  ...
  const boundary = createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && url.pathname === '/hooks/contentkit-notifications') {
      webhookEvents.push({ headers: req.headers, body: (await readBody(req)).toString('utf8') })
      res.writeHead(200, { 'content-type': 'application/json' })
      return res.end('{}')
    }
    res.writeHead(404, { 'content-type': 'application/json' })
  ...
  const child = spawn(binary, [], {
    env: {
      ...
      CONTENTKIT_SUBKIT_WEBHOOK_URL: `${boundaryOrigin}/hooks/contentkit-notifications`,
      CONTENTKIT_SUBKIT_WEBHOOK_SECRET: webhookSecret,
  ...
  try {
    ...
    const site = await responseJson(await fetch(`${origin}/v1/sites`, {
      method: 'POST',
...
`
    ...
    const ingested = await responseJson(await fetch(`${origin}/v1/sites/${site.id}/content`, {
      method: 'POST',
      headers: auth,
      body: multipart,
    }), 201)
    ...
    const preview = await responseJson(await fetch(`${origin}/v1/sites/${site.id}/previews`, {
      method: 'POST',
      headers: { ...auth, 'content-type': 'application/json' },
    ...
    const release = await responseJson(await fetch(`${origin}/v1/sites/${site.id}/releases`, {
      method: 'POST',
      headers: { ...auth, 'content-type': 'application/json' },
    ...
    const contact = await responseJson(await fetch(`${origin}/public/v1/contact`, {
      method: 'POST',
    ...
    const comment = await responseJson(await fetch(`${origin}/public/v1/posts/${ingested.item.id}/comments`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        site_id: site.id,
    ...
    const approved = await responseJson(await fetch(`${origin}/v1/comments/${comment.id}`, {
      method: 'PATCH',
      headers: { ...auth, 'content-type': 'application/json' },
    ...
    const webhooks = await waitForWebhooks(webhookEvents, 3)
    assert.deepEqual(new Set(webhooks.map((event) => event.headers['webhook-type'])), new Set([
    ...
    try {
      const state = await pool.query(`
        SELECT
          ...
          (SELECT count(*)::int FROM public.ck_outbox_events WHERE status = 'delivered') AS delivered_events
      `)
