...
function fixture() {
  ...
  const db = {
    ...
    async remove(table, f) {
      ...
      if (table === 'ck_deck_build_events') removedDeckEvents.push(f.created_at.slice(4))
    },
  ...
  return { db, storage, now, removedObjects, removedReleases, removedAuthEvents, removedDeckEvents, updated }
}
